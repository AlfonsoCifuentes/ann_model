  ...
  return (
    <div className="space-y-6">
            ...
            {heroPhotos.map((photo, index) => (
              <div key={photo._id} className="group relative">
                <div className="relative aspect-[4/5] rounded-lg overflow-hidden bg-fashion-bg-tertiary">
                  ...
                  {/* Order Number */}
                  <div className="absolute top-2 left-2 bg-fashion-rose text-fashion-bg rounded-full w-6 h-6 flex items-center justify-center text-xs font-bold">
                    {index + 1}
                  </div>
                </div>
                <div className="mt-2">
                  <p className="text-sm font-medium text-fashion-fg truncate">
                    {photo.title}
                  </p>
      ...
      <div className="bg-fashion-bg-secondary rounded-lg border border-fashion-bg-tertiary p-6">
        ...
        {photos.length === 0 ? (
          <div className="text-center py-8 text-fashion-fg-muted">
            No hay fotos disponibles. Sube algunas fotos primero.
          </div>
        ) : (
                ...
                <div className="relative aspect-[4/5] rounded-lg overflow-hidden bg-fashion-bg-tertiary">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-200">
                    <button
                      ...
                      className="bg-fashion-rose hover:bg-fashion-deep-copper text-fashion-bg px-4 py-2 rounded-full text-sm font-medium disabled:bg-fashion-fg-muted disabled:text-fashion-bg-secondary"
                    >
                      {heroPhotos.length >= 5 ? 'Hero Lleno' : 'Agregar al Hero'}
                    </button>
                  </div>
                </div>
                <div className="mt-2">
      ...
      <div className="bg-fashion-bg-tertiary border border-fashion-rose/30 rounded-lg p-4">
        <h4 className="font-semibold text-fashion-rose mb-2">💡 Instrucciones</h4>
        <div className="text-sm text-fashion-fg-secondary space-y-1">
          ...
          <p>• Puedes quitar fotos pasando el mouse sobre ellas</p>
        </div>
      </div>
      ...
      {saving && (
        <div className="text-center py-4">
          <div className="text-fashion-rose">Guardando cambios...</div>
        </div>
      )}
    </div>
  );
